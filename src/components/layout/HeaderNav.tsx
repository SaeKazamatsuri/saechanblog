	...
	return (
		<nav className="px-4">
			...
			{mounted ? (
				<>
					<div
						className={`fixed inset-0 z-50 bg-black/50 transition-opacity duration-200 ${open ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
							}`}
						onClick={closeAll}
					/>
					<aside
						ref={menuRef}
						id="mobile-menu"
						className={`fixed top-0 left-0 z-50 h-full w-4/5 max-w-sm bg-gray-900 shadow-xl transition-transform duration-200 ${open ? 'translate-x-0' : '-translate-x-full'
							}`}
					...
					>
						...
						<div className="px-2 py-2">
							...
							<div className={`${openCat ? 'max-h-96' : 'max-h-0'} overflow-hidden transition-[max-height] duration-200`}>
								<div className="pl-3">
									<Link
										href="/post"
							...
							<div className={`${openAbout ? 'max-h-96' : 'max-h-0'} overflow-hidden transition-[max-height] duration-200`}>
								<div className="pl-3">
									{[
										{ label: 'サイト運営について', href: '/charge-of-this-site' },
										{ label: 'サイトマップ', href: '/sitemap' },
