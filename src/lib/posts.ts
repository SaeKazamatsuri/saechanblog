...
export async function getPostData(categorySlug: string, id: string): Promise<PostData> {
    const dirName = fs.readdirSync(postsDirectory).find((d) => d.match(/^\[.+?\]\((.+?)\)$/)?.[1] === categorySlug)
